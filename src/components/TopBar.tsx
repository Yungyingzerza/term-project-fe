      ...
      <div className="max-w-7xl mx-auto px-4 h-full flex items-center gap-3">
        ...
        <div className="flex-1 max-w-xl mx-auto">
          <div className="group relative">
            ...
            <kbd className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] px-1.5 py-0.5 bg-white/10 rounded-md text-white/60">/</kbd>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button className="hidden sm:flex items-center gap-2 px-3 py-2 rounded-xl bg-white text-black font-semibold hover:opacity-90">
            <Upload className="w-4 h-4" /> Upload
          </button>
          <button className="p-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10">
            <Plus className="w-4 h-4" />
          </button>
          <img src="https://i.pravatar.cc/80?img=5" className="w-8 h-8 rounded-full" />
        </div>
      </div>
    </header>
  );
