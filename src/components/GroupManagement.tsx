...
export function GroupManagement({
  ...
  const { updateGroup, leaveGroup, isLoading: groupLoading } = useGroupManagement();
  const {
    invites,
    createInvite,
    revokeInvite,
  ...
  const handleRevokeInvite = async (inviteCode: string) => {
    if (!confirm("คุณแน่ใจหรือไม่ว่าต้องการเพิกถอนรหัสเชิญนี้?")) {
      return;
    }
  ...
  const handleRemoveMember = async (userId: string, username: string) => {
    if (
      !confirm(
        `คุณแน่ใจหรือไม่ว่าต้องการลบ ${username} ออกจากกลุ่มนี้?`
      )
    ) {
      return;
    }
    ...
    <div className="space-y-6">
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h3 className="text-xl font-semibold mb-1">{organization.name}</h3>
                <p className="text-white/70">{organization.description || "ไม่มีคำอธิบาย"}</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="bg-neutral-900/60 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl p-4">
              <h3 className="font-semibold mb-3 text-sm">สร้างรหัสเชิญใหม่</h3>
            ...
            {/* Active Invites List */}
            <div>
              <h3 className="font-semibold mb-3 text-sm">รหัสเชิญที่ใช้งานได้</h3>
              {invites.length === 0 ? (
                <p className="text-white/60 text-sm text-center py-8">ยังไม่มีรหัสเชิญ</p>
              ) : (
                    ...
                    return (
                      <div
                        ...
                        className={`flex items-center justify-between bg-white/5 border rounded-xl p-3 ${
                          isInactive ? "border-white/5 opacity-60" : "border-white/10"
                        }`}
                      >
                        <div className="flex-1">
                        ...
                        <div className="flex gap-2">
                          ...
                          {invite.is_active && (
                            <button
                              onClick={() => handleRevokeInvite(invite.invite_code)}
                              disabled={invitesLoading}
            ...
            {members.map((member) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-sm">{member.user_id.username}</p>
      ...
      <div className="bg-neutral-900/60 backdrop-blur-xl border border-red-500/20 rounded-2xl p-6">
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-red-400 mb-1">โซนอันตราย</h2>
